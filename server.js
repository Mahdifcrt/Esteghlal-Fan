...
if (!fs.existsSync(dbFile)) fs.writeFileSync(dbFile, JSON.stringify([
  {id:1,title:"استقلال برای دیدار بعدی آماده می‌شود",category:"تیم",body:"این یک خبر نمونه است. از پنل مدیریت می‌توانید این خبر را ویرایش یا حذف کنید.",image:"",breaking:true,date:new Date().toISOString()}
], null, 2));
...
app.get("/api/news",(req,res)=>{
  const items = readNews().sort((a,b)=>new Date(b.date)-new Date(a.date));
  res.json(items);
});
...
app.put("/api/news/:id",auth,(req,res)=>{
  const items=readNews(); const i=items.findIndex(x=>x.id==req.params.id);
  if(i<0) return res.status(404).json({error:"خبر پیدا نشد"});
  items[i]={...items[i],...req.body,id:items[i].id,date:items[i].date};
  writeNews(items); res.json(items[i]);
});
app.delete("/api/news/:id",auth,(req,res)=>{
  const items=readNews(); const i=items.findIndex(x=>x.id==req.params.id);
  if(i<0) return res.status(404).json({error:"خبر پیدا نشد"});
  const [deleted]=items.splice(i,1); writeNews(items); res.json(deleted);
});
app.get("/admin",(_,res)=>res.sendFile(path.join(__dirname,"public","admin.html")));
app.listen(PORT,()=>console.log(Esteghlal Fan running on http://localhost:${PORT}));
PS C:\Users\Mahdi\Desktop\esteghlal-fan-real-site> node server.js
Esteghlal Fan running on http://localhost:3000
PS C:\Users\Mahdi\Desktop\esteghlal-fan-real-site> git status
...
PS C:\Users\Mahdi\Desktop\esteghlal-fan-real-site> git log -1 --oneline
git : The term 'git' is not recognized as the name of a cmdlet, function, script file, or operable program. Check the
spelling of the name, or if a path was included, verify that the path is correct and try again.
At line:1 char:1
+ git log -1 --oneline
